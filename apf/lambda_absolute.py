...
component to within 0.035 decades (8%), with zero free parameters and
...
# (standard Planck-mass convention M_Pl = 1.22091e28 eV).
#
# Derivation from Planck 2018 TT,TE,EE+lowE+lensing best-fit:
#   H_0 = 67.36 km/s/Mpc = 1.4369e-33 eV
#   H_0 / M_Pl = 1.1769e-61
#   rho_crit / M_Pl^4 = (3/8pi)(H_0/M_Pl)^2 = 10^(-122.782)
#   Omega_Lambda = 0.6889 → rho_Lambda/M_Pl^4 = 10^(-122.944)
#   Omega_b = 0.0493      → rho_b/M_Pl^4       = 10^(-124.089)
#   Omega_c = 0.2618      → rho_c/M_Pl^4       = 10^(-123.364)
#
# Prior code used log values derived from an incorrect
# rho_vac ~ 2.8e-11 eV^4 look-up which gave rho_Lambda/M_Pl^4 ~
# 10^(-122.898); the honest natural-units value from the Planck 2018
# parameters is 10^(-122.944). Residuals therefore shift from the
# reported 0.012-0.013 decades to 0.033-0.035 decades (~8% vs
# observation, not 3%). Paper 8 reports the honest 8% number and
# flags the H_0 implication below (check_T_Lambda_to_H0_inversion).
_OBS_LOG10 = {
    'rho_crit':   -122.782,
    'rho_b':      -124.089,
    'rho_c':      -123.364,
...
def check_T_Lambda_absolute_extended_formula():
    ...
    return _result(
        ...
        summary=(
            ...
            "rho_b, rho_c, rho_Lambda) to within 0.035 decades (8%) "
            ...
            "Universal four-component agreement at 8% with zero free "
            "parameters."
        ),
        key_result=(
            'rho_X / M_Pl^4 = C_X / 102^62 matches all four observed '
            'cosmological densities (crit, b, c, Lambda) at 8%.'
        ),
...
def check_T_Lambda_coefficient_degeneracy_audit():
    ...
    matches observed rho_Lambda/M_Pl^4 to 0.034 decades. This audit
    ...
    C_vacuum/d_eff = 42/102 at 0.034 decades (factor 1.028).
    ...
    Representative top candidates from the scan:
        ...
        ...
        C_vacuum/d_eff = 42/102           residual 0.034 decades
    ...
    match observation at 8% precision. If the structural arguments
    for the coefficient are defensible, the 8% match is strong
    ...
    return _result(
        ...
        summary=(
            ...
            f"principles, not by numerical best-fit; the 8% observational "
...
# =============================================================================
# §5b  H_0 inversion: what Hubble constant does the APF formula predict?
# =============================================================================
...
def check_T_Lambda_to_H0_inversion():
    ...
    predictions at the Standard-Model interface:

        (a) Omega_Lambda       = C_vacuum / K_SM = 42 / 61
        (b) rho_Lambda / M_Pl^4 = C_vacuum / d_eff^(K_SM + 1) = 42 / 102^62

    Combining with the flat-universe critical-density definition
        rho_crit = 3 H_0^2 / (8 pi G) = 3 H_0^2 M_Pl^2 / (8 pi)
    [standard Planck-mass convention, G = 1/M_Pl^2], the Hubble constant
    is algebraically determined:

        rho_crit / M_Pl^4  =  rho_Lambda / (M_Pl^4 Omega_Lambda)
                           =  (42 / 102^62) / (42 / 61)
                           =  61 / 102^62
                           =  (3 / 8 pi) * (H_0 / M_Pl)^2

    Inverting:
        H_0 / M_Pl  =  sqrt( (8 pi / 3) * 61 / 102^62 )
                    =  sqrt( 8 pi * 61 / 3 ) / 102^31

    Numerically this gives H_0_APF = 70.03 km/s/Mpc, compared to:
        Planck 2018 (TT,TE,EE+lowE+lensing):   H_0 = 67.36 km/s/Mpc
        SH0ES (Riess et al. 2022):             H_0 = 73.04 km/s/Mpc
        Midpoint of the Hubble tension:        H_0 = 70.20 km/s/Mpc

    The APF prediction lies 0.17 km/s/Mpc from the exact midpoint of the
    Hubble tension, 2.67 km/s/Mpc above Planck and 3.01 km/s/Mpc below
    SH0ES. Interpretation: if the Hubble tension reflects a genuine
    physical effect rather than systematics at either end, APF predicts
    the resolution lands essentially at the tension midpoint, with zero
    free parameters.

    STATUS. [P] as a derived prediction from two [P] inputs plus the
    external GR rho_crit formula. The external-input status matches the
    standard-Planck-mass convention status: APF uses G (gravitational
    coupling) as external, and the rho_crit formula follows from the
    Einstein field equations in the FLRW metric. No additional
    assumptions are added for the inversion. The H_0 prediction is
    therefore at the same epistemic level as rho_Lambda/M_Pl^4 itself.

    HONEST CAVEAT. This is a post-hoc interpretation of the 8% residual
    between APF's rho_Lambda and the Planck 2018 observed value. The
    residual is NOT the observational uncertainty on rho_Lambda (Planck
    2018 gives Omega_Lambda to 0.8% precision); it is a genuine
    prediction-vs-observation discrepancy at 8%. If this 8% discrepancy
    is driven by a systematic uncertainty in H_0 itself (which varies by
    ~9% between Planck and SH0ES), then the APF prediction becomes "H_0
    lands where the tension resolves." This is not a derivation of the
    Hubble tension's resolution; it is a cross-check that APF's formula
    is consistent with the tension midpoint and therefore with any
    physics that resolves the tension at ~70 km/s/Mpc. Future tighter
    H_0 measurements (from JWST, LIGO standard sirens, delay-time
    cosmography) will either confirm the prediction (H_0 converges to
    ~70) or falsify it (H_0 converges to one endpoint of the current
    tension).

    DEPENDENCIES: L_Lambda_absolute_numerical_formula (rho_Lambda
    prediction), L_Omega_Lambda_is_entropy_fraction (Omega_Lambda
    prediction), L_count, L_self_exclusion.
    """
    K_SM = _CANON_K_SM
    d_eff = _CANON_D_EFF_SM
    C_vacuum = _CANON_C_VACUUM
    M_Pl_eV = 1.22091e28
    km_s_Mpc_to_eV = 2.1332e-35

    # APF-forced inputs
    Omega_Lambda_APF = C_vacuum / K_SM  # = 42/61
    log10_rho_Lambda_over_Mpl4_APF = (_math.log10(C_vacuum)
                                      - (K_SM + 1) * _math.log10(d_eff))
    log10_rho_crit_over_Mpl4_APF = (_math.log10(K_SM)
                                    - (K_SM + 1) * _math.log10(d_eff))
    # H_0 inversion
    # (H_0/M_Pl)^2 = (8 pi / 3) * K_SM / d_eff^(K_SM+1)
    # H_0/M_Pl    = sqrt(8 pi K_SM / 3) / d_eff^((K_SM+1)/2)
    H_over_Mpl_sq = (8 * _math.pi / 3) * K_SM / (d_eff ** (K_SM + 1))
    H_over_Mpl = _math.sqrt(H_over_Mpl_sq)
    H_APF_eV = M_Pl_eV * H_over_Mpl
    H_APF_km_s_Mpc = H_APF_eV / km_s_Mpc_to_eV

    # Reference values
    H_Planck_2018 = 67.36
    H_SH0ES_2022 = 73.04
    midpoint = (H_Planck_2018 + H_SH0ES_2022) / 2

    delta_planck = H_APF_km_s_Mpc - H_Planck_2018
    delta_sh0es = H_APF_km_s_Mpc - H_SH0ES_2022
    delta_midpoint = H_APF_km_s_Mpc - midpoint
    tension_width = H_SH0ES_2022 - H_Planck_2018

    # Sanity checks — every step of the derivation is internally
    # consistent.
    omega_sanity = abs(Omega_Lambda_APF - 42/61) < 1e-12
    rho_sanity = abs(_math.log10(Omega_Lambda_APF) +
    ...
    return _result(
        ...
        summary=(
            ...
            f"10^{log10_rho_Lambda_over_Mpl4_APF:.3f} — combined with the "
            f"standard GR critical-density formula rho_crit = 3 H_0^2 "
            f"M_Pl^2 / (8 pi), invert algebraically to give "
            f"H_0_APF = sqrt(8 pi * 61 / 3) / 102^31 * M_Pl = "
            f"{H_APF_km_s_Mpc:.2f} km/s/Mpc. Reference values: Planck 2018 "
            f"= {H_Planck_2018} km/s/Mpc (APF {delta_planck:+.2f}); SH0ES "
            f"2022 = {H_SH0ES_2022} km/s/Mpc (APF {delta_sh0es:+.2f}); "
            f"tension midpoint = {midpoint:.2f} km/s/Mpc (APF "
            f"{delta_midpoint:+.2f}). The APF prediction lies "
            f"{abs(delta_midpoint):.2f} km/s/Mpc from the midpoint of the "
            f"{tension_width:.2f}-km/s/Mpc-wide Hubble tension band — "
        ...
        key_result=(
            f'APF H_0 = {H_APF_km_s_Mpc:.2f} km/s/Mpc, '
            f'{abs(delta_midpoint):.2f} km/s/Mpc from Hubble tension midpoint.'
        ),
        dependencies=['L_Lambda_absolute_numerical_formula',
                      'L_Omega_Lambda_is_entropy_fraction',
        ...
        artifacts={
            ...
            'd_eff': d_eff,
            'C_vacuum': C_vacuum,
            'Omega_Lambda_APF_exact': '42/61',
            'Omega_Lambda_APF_float': Omega_Lambda_APF,
            'log10_rho_Lambda_Mpl4_APF': log10_rho_Lambda_over_Mpl4_APF,
            'log10_rho_crit_Mpl4_APF': log10_rho_crit_over_Mpl4_APF,
            'H_over_Mpl_APF': H_over_Mpl,
            'H_APF_eV': H_APF_eV,
            'H_APF_km_s_Mpc': H_APF_km_s_Mpc,
            'H_Planck_2018_km_s_Mpc': H_Planck_2018,
            'H_SH0ES_2022_km_s_Mpc': H_SH0ES_2022,
            'Hubble_tension_midpoint': midpoint,
            'tension_width_km_s_Mpc': tension_width,
            'APF_minus_Planck': delta_planck,
            'APF_minus_SH0ES': delta_sh0es,
            'APF_minus_midpoint': delta_midpoint,
            'within_tension_band': within_tension,
            'omega_sanity': omega_sanity,
            'rho_sanity': rho_sanity,
            'inversion_sanity': inversion_sanity,
            'all_OK': all_OK,
            'caveat': (
                'Post-hoc interpretation of the 8% rho_Lambda residual. '
                'Falsifier: future tighter H_0 measurements converging '
                'to ~70 km/s/Mpc confirm; converging to 67 or 73 '
                'falsify.'),
        },
    )
...
def check_T_Lambda_absolute_bulletproof():
    ...
    42/102^62 matches observation to 8% and is robust against
    ...
    return _result(
        ...
        key_result=(
            'APF predicts rho_Lambda/M_Pl^4 = 42/102^62 at 8% match '
...
_CHECKS = {
    ...
    # §5b Hubble-constant inversion (1 [P], tier 4) — Phase 14e.3
