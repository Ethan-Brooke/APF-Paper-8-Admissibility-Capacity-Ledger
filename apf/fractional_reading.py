...
def check_L_N_SM_hierarchy_near_miss():
    ...
    # convention. Derived from Planck 2018 TT,TE,EE+lowE+lensing
    # parameters (H_0 = 67.36 km/s/Mpc, Omega_Lambda = 0.6889):
    #   (a) Standard Planck mass M_Pl = 1.22091e28 eV (gravitational
    #       coupling G = 1/M_Pl^2, "heavy Planck mass"; this is the
    #       "120+ orders of magnitude" cliche of the cosmological-
    #       constant problem):
    #       log10(rho_Lambda / M_Pl^4) = -122.944.
    #   (b) Reduced Planck mass M_Pl_red = M_Pl / sqrt(8 pi)
    #       (natural for effective field theory; (8 pi)^2 ~ 632
    #       factor shifts log10 by +2.800):
    #       log10(rho_Lambda / M_Pl_red^4) = -120.144.
    # APF's bare 1/N_SM = 10^-122.525 matches (a) to within 0.419
    # decades and (b) to within 2.381 decades, so the standard-
    # Planck-mass convention is by far the closer match. The bare-
    # 1/N_SM prediction is superseded by check_L_Lambda_absolute_
    # numerical_formula which identifies the coefficient 42/102 and
    # brings the residual to 0.034 decades (8% vs observation).
    obs_standard = -122.944
    obs_reduced = -120.144
...
def check_L_Lambda_absolute_numerical_formula():
    ...
    # Observed (Planck 2018 TT,TE,EE+lowE+lensing best-fit cosmological
    # parameters; standard Planck-mass convention, i.e. G = 1/M_Planck^2
    # with M_Planck = 1.22091e28 eV). Derivation:
    #   H_0        = 67.36 km/s/Mpc = 1.4369e-33 eV
    #   H_0/M_Pl   = 1.1769e-61
    #   rho_crit   = 3 H_0^2 M_Pl^2 / (8 pi),  so
    #   rho_crit/M_Pl^4  = (3/8pi) (H_0/M_Pl)^2 = 1.6534e-123
    #                    = 10^(-122.782)
    #   rho_Lambda/M_Pl^4 = Omega_Lambda * rho_crit/M_Pl^4
    #                     = 0.6889 * 1.6534e-123 = 1.1390e-123
    #                     = 10^(-122.9435).
    # Prior code used log10_obs = -122.898 based on an incorrect
    # rho_vac ~ 2.8e-11 eV^4 look-up; the correct natural-units value
    # derived from the Planck 2018 cosmological parameters + standard
    # Planck-mass convention is rho_Lambda ~ 2.530e-11 eV^4, giving
    # log10 = -122.944. Honest residual is therefore 8%, not 3%.
    # Paper 8 reports the honest 8% number and flags the H_0 implication
    # (cf. check_T_Lambda_to_H0_inversion in apf/lambda_absolute.py).
    log10_obs = -122.944
