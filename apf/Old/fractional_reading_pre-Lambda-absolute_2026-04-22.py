...
Research question registered as an observation (not a theorem)
--------------------------------------------------------------
The reciprocal of the SM total microstate count 1/N_SM = 102^(-61)
is numerically 10^(-122.53). The observed rho_vac / M_Planck^4 is
approximately 10^(-122.90) with standard Planck-mass convention
(the "120 orders of magnitude" cliche from the cosmological-
constant problem) and 10^(-120.10) with reduced Planck-mass
convention. The APF 1/N_SM prediction matches the standard-Planck
convention to within 0.37 decades (factor 2.4), which is
strikingly close for a zero-free-parameter expression in a
framework derived from A1 alone. This suggestive numerical
proximity is recorded in `check_L_N_SM_hierarchy_near_miss` as a
live research question consistent with FRE + the two-tier
framework; it is explicitly NOT claimed as a derivation of Lambda
absolute. An honest Lambda-absolute derivation from FRE would
require fixing the Planck-mass convention from structural
arguments and identifying the residual O(1) coefficient from A1
...
_CHECKS = {
    ...
    # §6  Hierarchy research observation (1 [C], tier 3)
