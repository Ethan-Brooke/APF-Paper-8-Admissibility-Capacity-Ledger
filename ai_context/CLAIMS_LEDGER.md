...
| 8 | Unique 42-dim $G_{\rm SM}$-invariant complement $V_\Lambda \subset V_{61}$. | nontrivial (pending) | local formal kernel | Supp §1.4–1.5 Theorem 1.1 (Maschke) | no standalone code check for the Maschke argument itself; `check_T_interface_sector_bridge` in gravity.py is the downstream witness | uniqueness depends on $V_{\rm local}$ being specified by irrep content, not by dimension alone. |
